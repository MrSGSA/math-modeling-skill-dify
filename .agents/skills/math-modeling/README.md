...
本 Skill 将数学建模任务拆分为 **建模分析 → 代码实现 → 红队评审 → 论文撰写** 四个阶段。既可以按顺序完成整道题，也可以只执行其中一个阶段。
...
- 🛡️ **红队评审门**：检查判据、反例、边际贡献、离散指派、随机稳定性、网格收敛、跨表一致性和最优性证据等级。
- 🔎 **双引擎论文搜索**：并行调用 OpenAlex 与 AnySearch，按 DOI 或题名交叉核验。
- 📄 **Word 论文生成**：支持官方模板、公式优先的结果计算链、Word 原生 OMML 公式、可编辑表格、篇幅与图文引用检查、结构校验和渲染抽检。
...
| ③ | [评审手](references/roles/model-reviewer/SKILL.md) | 独立红队审计、反例测试和证据等级控制 | 内部结果注册表与审计文件 |
| ④ | [论文手](references/roles/论文手/SKILL.md) | 基于已通过评审的真实结果生成 Word 论文 | `完整论文.docx` |

### 阶段反馈

- 编程手发现公式、约束或参数无法实现时，携带实际报错返回建模手修正。
- 评审手发现重大判据、实现或证据问题时，按审计结果返回建模手或编程手修正。
...
| [PDF 批量转换](tools/pdf-batch/SKILL.md) | 使用 MinerU 将论文、扫描件和公式批量转为知识库 Markdown |
...
CUMCM默认以摘要渲染后接近写满且不溢出一页、正文恰好30页作为优选完整度目标，但这不是官方最低要求，且不得靠空话、放大图表、强制章节分页或制造空白凑页。一级标题默认连续排版。以2026年官方规范为例，摘要原则上不超过一页、正文不超过30页、附录页数不限；电子论文没有总页数上限，但单文件不超过20MB。实际交付必须重新核对目标届次的官方文件。校验器还会检查公式、图、表的基本数量，图表题注与正文引用，以及参考文献和正文引用是否双向对应。图形除真实结果图外，也可包含由Python或矢量工具生成、且与最终模型一致的算法流程图和解释图。
