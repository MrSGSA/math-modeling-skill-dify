...
def build_manifest(
    ...
    if not str(command).strip():
        raise ValueError("reproduce_command 不能为空")
    files = []
    for item in inputs:
        ...
        if root is not None and not _is_within(path, root):
            raise ValueError(f"输入文件必须位于 PROJECT_ROOT 内: {path}")
        recorded_path = path.relative_to(root).as_posix() if root is not None else str(path)
        files.append({"path": recorded_path, "sha256": sha256_file(path), "bytes": path.stat().st_size})
    ...
    elif not str(runtime_version or "").strip():
        raise ValueError(f"{runtime_name} 运行时必须显式记录版本")
...
def resolve_output(project_root, relative_output):
    ...
    if not project.is_dir():
        raise ValueError(f"PROJECT_ROOT 不存在或不是目录: {project}")
