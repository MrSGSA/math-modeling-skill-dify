...
- 论文生成改为“公式优先、文字从简”：关键结果按定义公式、估计/优化公式、数值代入或注册表来源、最终结果组织。
- 联合目标、评价指标、误差、置信区间、灵敏度、收敛指标和模型比较量优先使用与真实代码一致的可编辑 Word OMML 公式展示。
- 禁止用等价变形或装饰公式凑数量；公式必须核对代码口径、单位、边界和 `result_registry.json`。
- AI 使用标识与披露文本改为参赛者依据当届官方规则人工填写、定位和核验，论文生成器不再自动批量插入。
- 更新论文结构回归测试，使30页正文优选目标与校验器规则保持一致。

## 1.4.0 - 2026-07-23

- 完整流程升级为“建模手 → 编程手 → 评审手 → 论文手”，在论文定稿前加入独立红队评审门。
- 新增几何/统计判据、极端反例、集合计量、资源删除边际、离散指派、多随机种子、网格收敛和跨表一致性检查。
- 新增 `result_registry.json` 单一结果源和 `red_team_audit.json` 审计格式，限制可行、当前预算较优、离散层全局最优与全局最优的措辞等级。
- 新增 `red_team_gate.py` 及回归测试；重大失败、待验证关键主张或无证明的全局最优声明将阻断论文定稿。
...
- 新增基于 MinerU 的 PDF 批量转 Markdown 工具，支持扫描件 OCR、LaTeX 公式、图片和表格输出。
- 新增 Windows 一键入口、递归发现、断点续跑、SHA-256 去重、逐文件日志和 Dify 手动上传清单。
- 默认使用支持纯 CPU 的 pipeline 后端；未获用户同意时不自动安装大体积依赖或下载模型。
- 增加批处理命令构造、PDF 发现和无 MinerU 干运行的回归测试。
