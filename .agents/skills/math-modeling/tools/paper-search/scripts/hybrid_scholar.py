...
class HybridScholar:
    ...
    def search_papers(
        ...
        source_status = {
            "openalex": "pending" if use_oa else "disabled",
            "anysearch": "pending" if use_any else "disabled",
        }
        ...
        with ThreadPoolExecutor(max_workers=2) as pool:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    print(f"[杂交] {source} 搜索异常: {e}", file=sys.stderr)
    ...
    def _fuse(self, oa_papers: List[Paper], any_papers: List[Dict],
              final_limit: int, source_status=None, source_errors=None) -> Dict[str, Any]:
        """融合两个源的结果，去重并标记交叉验证。"""
        ...
        source_status = dict(source_status or {
            "openalex": "ok" if oa_papers else "not_observed",
            "anysearch": "ok" if any_papers else "not_observed",
        })
        ...
        return {
            ...
            "stats": {
                ...
                "source_status": source_status,
                "source_errors": source_errors,
                "dual_engine_cross_validation_complete": (
                    source_status.get("openalex") == "ok"
                    and source_status.get("anysearch") == "ok"
                ),
                "degraded": "failed" in source_status.values(),
            },
        }
    ...
    def _tokens(cls, text: str) -> List[str]:
        ...
        return [cls._normalize_token(token) for token in normalized.split()]
    ...
    def _query_terms(cls, query: str) -> List[str]:
        ...
        return [term for term in cls._tokens(query) if term not in stopwords and len(term) > 1]

    @classmethod
    def _paper_text(cls, paper: HybridPaper) -> tuple[str, str]:
        title = " ".join(cls._tokens(paper.title))
        detail = " ".join(cls._tokens(" ".join(filter(None, [paper.abstract, paper.venue]))))
