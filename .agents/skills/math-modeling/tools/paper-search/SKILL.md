...
诊断单个引擎时可用 `--openalex-only` 或 `--anysearch-only`。正式文献检索默认运行两个引擎；若某一引擎因鉴权、网络或服务故障不可用，在记录失败原因后可降级使用另一真实数据源，并把结果标为未完成双引擎交叉验证，禁止伪造缺失引擎的命中。机器可读输出中的 `stats.source_status`、`stats.source_errors`、`stats.degraded` 和 `stats.dual_engine_cross_validation_complete` 必须随结果保留。
