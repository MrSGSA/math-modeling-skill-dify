...
AI 工具使用标识、正文标注、参考文献条目和使用详情说明不得由论文生成程序自动写入成稿。论文手只在内部检查中提醒：参赛者必须依据目标竞赛当届官方规则，人工决定标注位置、填写真实工具信息、核对使用详情并承担最终责任。用户未明确提供已人工确认的披露文本时，自动生成的论文保持无 AI 标识状态，不得用统一段末标记批量污染正文，也不得把模板化披露冒充参赛者确认。
...
论文正文采用“公式优先、文字从简”的结果展示方式。每个关键结果至少给出定义公式、估计或优化公式、必要的数值代入式（或明确的结果注册表来源）和最终结果，不能只在表格或段落中直接报数。联合目标、评价指标、误差、置信区间、灵敏度、收敛指标和模型比较量均应尽量显式公式化。公式必须与真实代码的计算口径、单位和边界一致，并使用可编辑 Word OMML；不得为了增加公式数量拆分同义公式、伪造推导或写与程序无关的装饰公式。复杂推理可用简短文字说明公式的物理意义、适用条件和结果解释。
