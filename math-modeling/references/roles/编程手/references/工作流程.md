...
根据用户要求、现有代码、模型结构和可用依赖选择 Python、MATLAB 或两者，并在开始编码前记录一句选择依据。不得仅因代理更熟悉 Python 就默认选择 Python。
...
| 传统竞赛数值题且两者均适合 | MATLAB | 符合常见竞赛工作流；若改选 Python，记录其数据生态、自动化或复现优势 |

用户明确偏好优先于上表。MATLAB 环境或必要工具箱不可用时，不得假装运行；应说明依赖情况后选择可真实复现的语言。只有需要独立数值交叉验证或用户明确要求时才双语言实现；语言选择不能替代算法正确性、误差验证和边界检查。
