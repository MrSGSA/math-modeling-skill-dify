...
def _body_document_texts(doc):
    """返回正文及其前置内容，遇到首个附录标题即停止。"""
    texts = []
    for child in doc._element.body.iterchildren():
        text = "".join(node.text or "" for node in child.iter(qn("w:t"))).strip()
        compact = re.sub(r"\s+", "", text)
        if child.tag == qn("w:p") and compact.startswith("附录") and len(compact) <= 80:
            break
        if text:
            texts.append(text)
    return texts
...
def validate_paper_structure(
    ...
    *,
    ...
    if contest.lower() == "cumcm":
        ...
        target_pages = 30 if target_pages is None else target_pages
        target_abstract_fill_ratio = (
            0.85 if target_abstract_fill_ratio is None else target_abstract_fill_ratio
        )
        official_max_pages = 30 if official_max_pages is None else official_max_pages
        official_max_file_size_mb = (
            20 if official_max_file_size_mb is None else official_max_file_size_mb
        )
    else:
    ...
    if require_abstract_fill_ratio is None:
        require_abstract_fill_ratio = require_rendered_pages

    body_text = "\n".join(_body_document_texts(doc))
    ...
    if rendered_body_pages is None and require_rendered_pages and target_pages is not None:
        errors.append(
            f"预警：未提供正文渲染页数，无法检查约 {target_pages} 页正文质量目标和 "
            f"{official_max_pages} 页正文官方上限；论文总页数不能替代正文页数"
        )
    elif rendered_body_pages is not None:
        if target_pages is not None and rendered_body_pages < target_pages:
            errors.append(
                f"预警：渲染后正文共 {rendered_body_pages} 页，低于约 {target_pages} 页质量目标；"
                "该目标不是官方最低页数"
            )
        if official_max_pages is not None and rendered_body_pages > official_max_pages:
            errors.append(
                f"渲染后正文共 {rendered_body_pages} 页，超过当前核验的正文官方上限 "
                f"{official_max_pages} 页"
            )
    if electronic_file_size_bytes is None:
        if require_electronic_file_size and official_max_file_size_mb is not None:
            errors.append("预警：未提供电子版论文文件大小，无法检查 20MB 官方上限")
        ...
        if electronic_file_size_bytes > limit:
            ...
            errors.append(
                f"电子版论文文件为 {size_mb:.2f}MB，超过官方上限 "
                f"{official_max_file_size_mb}MB"
            )
    if abstract_fill_ratio is None:
        if require_abstract_fill_ratio and target_abstract_fill_ratio is not None:
            errors.append(
                "预警：未提供摘要页渲染填充率，无法检查摘要是否内容充实且未溢出一页"
            )
    else:
        if abstract_fill_ratio > 1:
            errors.append("摘要渲染内容超过一页可用区域")
        elif (
            target_abstract_fill_ratio is not None
            and abstract_fill_ratio < target_abstract_fill_ratio
        ):
            errors.append(
                f"预警：摘要页有效区域填充率约 {abstract_fill_ratio:.0%}，低于 "
                f"{target_abstract_fill_ratio:.0%} 的质量目标；应补充有效信息而非机械凑字"
            )
    return errors
